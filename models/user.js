        ...
        if (user) {
            if (user.isVerified === false) {
              ...
              try {
                ...
                res.status(201).json({ "msg": "Verification link sent again. Please check your email for the verification link. !!" });
                return;
              } catch (err) {
        ...
        if(user) 
        {
            await bcrypt.compare(req.body.password,user.password,async (err,same) => {
                 if(!err && same) {
                    ...
                    console.log(key);
                    const cipher=await crypto.createCipheriv(algorithm,toString(process.env.secretKey));
     ...
     if(req.user) 
     {
            try {
                ...
                await req.user.tokens.map(async (token)=> {
                     ...
                     const decipher= await crypto.createDecipheriv(algorithm,toString(process.env.secretKey));
